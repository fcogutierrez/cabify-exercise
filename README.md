...
In order to run locally this application, go to `Cabify.CarPooling.Api` folder and run from console the command `dotnet run`. 
Then, open `https://localhost:5001` from your browser if you want to make request to the API on an easy way (using the Swagger interface).

Some remarks:
* The `status` method will execute a Health Check strategy in order to make sure that all the infrastrcuture is up-and-running. If everything goes fine, it returns a `200` status code, otherwise it would return `500`.
* Once a Journey is dropped, it is kept at the database, but it will not be found when trying to locate it/drop it from the API.

Technical details:
* Backend implementation with `.NET Core 6`.
* Using a `Sqlite` in-memory database in order to persist the state. Since `.NET Core` does not offer an easy way to perform an stateful API, I had to go for this approach.
* Using the Dependency Injection pattern (implementation of the Dependency Inversion principle) basing on the .NET Core built-in container.
* Using the CQS pattern (command/query segregation) in order to separate write from read operations.
* Using the Decorator pattern in order to log all the processed Commands and Queries, and possible exceptions.
* Using the Unit Of Work pattern in order to make all the operations on the different tables under a single transaction.
* Using Swagger in order to provide an UI-friendly way to send requests to the API.
* Implementing a Clean Architecture approach, where the internal layers do not depend on outer layers, and both Application and Domain are infrastructure-agnostic.
* Based on a DDD approach, using encapsulated entities that expose methods to change their state.